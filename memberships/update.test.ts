...
Deno.test("PUT /memberships/:id.json", async (t) => {
  await t.step("if got 204, should resolve", async () => {
    ...
    await expect(update(context, 5, { roleIds: [2] })).resolves
      .toBeUndefined();
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      await expect(update(context, 422, { roleIds: [2] })).rejects.toThrow();
    },
  );

  await t.step("if get invalid response with unexpected format", async () => {
    server.use(...invalidHandlers);
    await expect(update(context, 404, { roleIds: [2] })).rejects.toThrow();
  });
