...
Deno.test("PUT /groups/:id.json", async (t) => {
  await t.step(
    "if got 204, should resolve",
    async () => {
      server.resetHandlers(...validHandlers);
      await expect(update(context, 20, { name: "Developers" })).resolves
        .toBeUndefined();
    },
  );
  ...
  await t.step(
    ...
    async () => {
      ...
      await expect(update(context, 422, { name: "Developers" })).rejects
        .toThrow();
    },
  );
  ...
  await t.step("if get invalid response with unexpected format", async () => {
    server.resetHandlers(...invalidHandlers);
    await expect(update(context, 404, { name: "Developers" })).rejects
      .toThrow();
  });
  ...
  await t.step(
    ...
    async () => {
      ...
      await update(context, 20, { userIds: [3, 5] });
